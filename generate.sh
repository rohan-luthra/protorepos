...
GIT_HOST="wtf-dot-menu"
GIT_REPO=${GIT_REPO-~/work/WTF/wtf-backend/repos}
GIT_BRANCH=${GIT_BRANCH-"master"}
GIT_TOKEN="366c68f4700919397cb04c18fb98d5a5da52480a"
  ...
  if [ -f .protolangs ]; then
    while read lang; do
      ...
      for protofile in proto/* ; do
          ...
          sudo docker run -v `pwd`:/defs namely/protoc-all -f $protofile -l $lang --with-docs --lint $([ $lang == 'node' ] && echo "--with-typescript" || echo "--with-validator")
      done
